    ...
    <section className="w-full bg-zinc-950 py-20 px-4 md:px-8 border-t border-white/[0.05] relative z-10">
      <div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {statsList.map((stat, i) => (
          <div 
            key={i} 
            className="cursor-crosshair p-6 bg-zinc-900/10 border border-white/[0.06] rounded-xl flex flex-col justify-between h-[180px] font-mono relative overflow-hidden transition-all duration-300 hover:border-white/15 hover:bg-zinc-900/20"
          >
            {/* Fila Superior */}
            ...
            {/* Bloque Central (Tipografía Cinzel original) */}
            <div className="flex flex-col my-auto">
              <span className="font-cinzel text-4xl md:text-5xl font-bold text-white tracking-tight leading-none mb-1">
                {stat.val}
              </span>
