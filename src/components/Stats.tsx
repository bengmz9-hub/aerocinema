		...
		<section className="w-full bg-transparent py-20 px-4 md:px-8 border-t border-white/[0.05] relative z-10">
			<div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
				{statsList.map((stat) => (
					<StarBorder
						key={stat.sys}
						color={stat.isRec ? "#ef4444" : "rgba(212, 175, 55, 0.4)"}
						speed={stat.isRec ? "3s" : "8s"}
						thickness="1px"
						className="w-full h-[180px]"
					>
						<div className="cursor-crosshair p-6 flex flex-col justify-between h-full font-mono relative overflow-hidden transition-all duration-300 hover:bg-white/[0.02]">
							{/* Fila Superior */}
