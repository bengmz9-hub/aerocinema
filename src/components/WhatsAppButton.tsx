...
export default function WhatsAppButton({
	...
	return (
		<a
			...
			className="fixed z-40 md:z-50 bottom-[calc(env(safe-area-inset-bottom)+1rem)] md:bottom-6 right-4 md:right-6 relative flex items-center justify-center md:justify-start gap-2.5 bg-[#12141a]/90 backdrop-blur-xl md:bg-[#12141a]/85 md:backdrop-blur-2xl border border-white/15 text-white h-12 w-12 md:h-auto md:w-auto md:pl-4 md:pr-4 md:py-3 rounded-full shadow-[0_8px_24px_rgba(0,0,0,0.5),0_0_28px_-6px_rgba(212,175,55,0.35)] hover:shadow-[0_8px_24px_rgba(0,0,0,0.55),0_0_36px_-4px_rgba(212,175,55,0.5)] hover:scale-105 active:scale-95 transition-all duration-300 group focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-200/70 min-h-[44px] min-w-[44px]"
		>
			<svg
			...
			<span className="hidden md:block text-[13px] font-bold tracking-wide md:pr-1">
				Contactar por WhatsApp
			</span>
			<span
				className="absolute md:relative bottom-0 right-0 translate-x-[30%] translate-y-[30%] md:translate-x-0 md:translate-y-0 h-2 w-2 md:h-2.5 md:w-2.5 shrink-0"
				aria-hidden="true"
			>
				<span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-emerald-400 opacity-60" />
				<span className="relative inline-flex h-2 w-2 md:h-2.5 md:w-2.5 rounded-full bg-emerald-400 shadow-[0_0_10px_2px_rgba(52,211,153,0.7)]" />
			</span>
		</a>
	);
}
